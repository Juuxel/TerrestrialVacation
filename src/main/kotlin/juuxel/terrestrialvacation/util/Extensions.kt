...
inline fun <T> Registry<T>.visit(crossinline visitor: (Identifier, T) -> Unit) {
    for (t in this) {
        visitor(getId(t) ?: throw IllegalStateException("Could not find ID for supposedly present registry entry: $t"), t)
    }

    RegistryEntryAddedCallback.event(this).register(RegistryEntryAddedCallback { _, id, value -> visitor(id, value) })
}
